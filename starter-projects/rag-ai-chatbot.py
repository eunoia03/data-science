...
prompt_template = ChatPromptTemplate.from_messages([ #from messages allows us to add in things like memory
    ("system",
...
_session_store = {} #create empty dicitonary (mini database, store conversation histories, with each session identified by unique session id)
def get_session_history(session_id: str) -> ChatMessageHistory:
    if session_id not in _session_store:
        _session_store[session_id] = ChatMessageHistory() # create new instance of chat message history
    return _session_store[session_id]

#this function is responsibile for creating and managing chat history for each conversation had with the ai system
...
resp1 = chain_with_history.invoke({"input": "Hi, I'm Daniel.  What is this Delivery Club all about?"}, config=memory_config)
print(resp1.content)
